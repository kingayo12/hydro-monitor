...
const Nutrientlevel = () => {
  ...
  const [nutrientLevel, setnutrientLevel] = useState(0.0);

  useEffect(() => {
    const targetnutrientLevel = 6.5; // Set the target Nutrient Level

    const interval1 = setInterval(() => {
      if (nutrientLevel < targetnutrientLevel) {
        setnutrientLevel((prevNutrient) => parseFloat((prevNutrient + 0.1).toFixed(1))); // Increment Nutrient Level by 0.1 with one decimal place
        const circleElement = circleRef.current;
        if (circleElement) {
          if (nutrientLevel < 5.5) {
            circleElement.setAttribute("stroke", "yellow");
            setnewStatus("low");
          } else if (nutrientLevel > 7.5) {
            circleElement.setAttribute("stroke", "red");
            setnewStatus("high");
          } else {
            circleElement.setAttribute("stroke", "blue");
            setnewStatus("good"); // Default color for Nutrient Level between 5.5 and 7.5
          }
        }
      } else {
        ...
        <div className=' flex flex-row  h-full'>
          <div className='right-d w-full h-screen pt-9 pl-5 my-5'>
            <div className='ph-level_header'>
              <h2 className='text-1xl uppercase font-bold primary-color3 my-5'>Nutrient level</h2>
            </div>
            <div className='phlevel flex flex-row'>
              <div className='phlevel-box mx-2'>
                <div class='heade-phr text-center text-sm capitalize'>
                  Nutrient Level on current time
                </div>
                <div class='pH-value'>
                  <div className='stroke3  rounded-full  relative'>
                    <div className='outer rounded-full'>
                      <div className='inner  rounded-full'>
                        <div className='number capitalize'>{nutrientLevel}</div>
                      </div>
                    </div>
